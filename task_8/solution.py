    ...
    if type == 0:
        for i in range(amountOfVectors):
            ...
            if xL + scale * matrixSizeX + xShift > 1200:
                yL += scale * matrixSizeY + yShift + 65
                if yL + scale * matrixSizeY + yShift + 65 > 1000:
                    break
                xL = xShift
    else:
        for i in range(amountOfVectors):
...
def displayOneVector(data, classes, weights, it, xL, yL, scale, draw, fnt, fnt2):
    ...
    for i in range(outputClasses):
        if weights[currentVec][i] == wMax:
            draw.text((currentX, currentY + (20 + 15 * i)), format(weights[currentVec][i], ".2f"), fill=(255, 0, 0),
                      font=fnt2)
        else:
            draw.text((currentX, currentY + (20 + 15 * i)), format(weights[currentVec][i], ".2f"), fill=(255, 0, 255),
                      font=fnt2)
...
inputLearning = "D:/inputLearning.txt"
inputTest = "D:/inputLearning.txt"
